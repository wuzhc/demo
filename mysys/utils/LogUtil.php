...
define('LOG_PATH', dirname(dirname(__FILE__)) . '/runtime/log/');
...
class LogUtil
{
    ...
    public static function w($msg, $mode = 'a+')
    {
        ...
        $fp = fopen(LOG_PATH . self::$logName, $mode);
        if (!$fp) {
            echo 'fopen() failed';
            return false;
        }
    ...
    {
        $res = @unlink(LOG_PATH . self::$logName);
        if ($hint) {
    ...
    public static function r()
    {
        ...
        $fp = fopen(LOG_PATH . self::$logName, 'r');
        if (!$fp) {
            echo 'fopen() failed';
            return false;
        }
